...
MIN_WEIGHT, MAX_WEIGHT = 50, 30000  # g
...
MIN_TOTAL_DIMENSIONS, MAX_TOTAL_DIMENSIONS = 26, 200  # cm
...
class ShippingLabel:
    ...
    def get_datamatrix_info(self):
        ...
        parts = [
            "{!s:0>8}".format(self.receiver.zip_code),
            "{:0>5}".format(self.receiver.zip_complement),
            "{!s:0>8}".format(self.sender.zip_code),
            "{:0>5}".format(self.sender.zip_complement),
            "{:01d}".format(self.receiver.zip_code.digit),
            "{:02d}".format(self.variable_data_identifier),
            "{!s:>13}".format(self.tracking_code),
            "{:>12}".format(self._get_extra_service_info()),
            "{:0>10}".format(self.posting_card.number),
            "{:0>5}".format(self.service.code),
            "{:02d}".format(self.posting_list_group),
            "{}".format(receiver_number),
            "{:0>20.20}".format(self.receiver.complement_safe_display),
            "{:05d}".format(0 if self.value is None else int(self.value * 100)),
            "{!s:0>12.12}".format(self.receiver.phone),
            "{:+010.6f}".format(self.latitude),
            "{:+010.6f}".format(self.longitude),
            "|",
            "{:<30.30}".format(self.text),
        ]
        return "".join(parts)
